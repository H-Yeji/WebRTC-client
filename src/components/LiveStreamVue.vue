...
export default {
  ...
  methods: {
    async startStream() {
      ...
      console.log('Offer sent:', this.peerConnection.localDescription);

      // 웹소켓 메시지 수신 핸들러 설정
      this.signalingSocket.onmessage = async (message) => {
        console.log('WebSocket message received:', message.data); // 출력됨 
        try {
          const data = JSON.parse(message.data);
          console.log('Parsed data:', data); // 출력됨 
    ...
    async joinStream() {
      ...
      this.peerConnection.ontrack = event => {
        ...
        if (event.streams[0]) {
          ...
          console.warn('No streams found in event:', event);
        }
      };
